...
async def generate_image(req: ImageRequest):
    print("[后端] 收到生图请求")
    try:
        enhanced_prompt = f"{req.prompt} 。【请严格生成 {get_ratio_desc(req.aspect_ratio)} 比例】"
        ...
        async with httpx.AsyncClient(timeout=120) as client:
            ...
            print(f"[后端] 上游状态码: {resp.status_code}")

            if resp.status_code != 200:
                error_text = resp.text[:600]
                print(f"[后端] 上游返回非200: {error_text}")
                raise HTTPException(500, f"上游错误: {error_text}")
        ...
        print("[后端] 发生异常，完整堆栈如下：")
        traceback.print_exc()
        raise HTTPException(500, f"后端处理失败: {str(e)}")
